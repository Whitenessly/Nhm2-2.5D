...
class MapLoader:
    """
    Utility for scanning, listing, and loading raw BenchNav map datasets (.pt / .npy) from map_data folder.
    Directly parses raw PyTorch map checkpoints from masafumiendo/benchnav.
    """
    ...
    def list_available_maps(self):
        """Scans map_data directory and returns list of raw map filenames (.pt and .npy)."""
        pattern_pt = os.path.join(self.map_data_dir, "*.pt")
        pattern_npy = os.path.join(self.map_data_dir, "*.npy")

        map_files = sorted(glob.glob(pattern_pt)) + sorted(glob.glob(pattern_npy))
        map_list = []
        for f in map_files:
    ...
    def select_map_interactive(self):
        ...
        if not map_files:
            raise FileNotFoundError(f"No raw map files (.pt or .npy) found in {self.map_data_dir}!")
    ...
    def load_map(self, filepath_or_name):
        """
        Loads raw 2.5D heightmap directly from BenchNav .pt file or .npy file.
        :return: X, Y, Z, resolution
        """
        if not (filepath_or_name.endswith(".pt") or filepath_or_name.endswith(".npy")):
