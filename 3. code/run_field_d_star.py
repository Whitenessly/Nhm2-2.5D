...
def run_field_d_star_standalone(map_name, start_pose=None, goal_pose=None):
    """Executes 2.5D Field D* on the given map name."""
    loader = MapLoader()
    ...
    print(f"Executing 2.5D Field D* search on '{map_name}' from {start_pose[:2]} to {goal_pose[:2]}...")

    planner = FieldDStar25D(hmap)
    ...
    metrics["algorithm"] = "2.5D Field D*"

    print("\n==========================================================================")
    print(f" KẾT QUẢ THỰC NGHIỆM 2.5D FIELD D* (Map: {map_name})")
    print("==========================================================================")
    ...
    json_path = os.path.join(log_dir, f"field_d_star_{map_name}.json")
    with open(json_path, "w") as f:
        json.dump(metrics, f, indent=2)

    img_path = os.path.join(log_dir, f"field_d_star_{map_name}.png")
    ...
    if path:
        pts = np.array(path)
        ax1.plot(pts[:, 0], pts[:, 1], pts[:, 2] + 0.1, color='purple', linestyle='-', linewidth=3, label='2.5D Field D* Path')
    ...
    ax1.set_title(f'2.5D Field D* 3D Trajectory ({map_name})', fontsize=12, fontweight='bold')
    ...
    if path:
        pts = np.array(path)
        ax2.plot(pts[:, 0], pts[:, 1], color='purple', linestyle='-', linewidth=2.5, label='2.5D Field D*')
...
def main():
    print("==========================================================================")
    print("      2.5D FIELD D* PLANNER (Ferguson & Stentz, 2005) - STANDALONE        ")
    print("==========================================================================")
    loader = MapLoader()
    map_name, filepath = loader.select_map_interactive()
    run_field_d_star_standalone(map_name)
