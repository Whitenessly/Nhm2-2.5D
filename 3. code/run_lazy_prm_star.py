...
def run_lazy_prm_star_standalone(map_name, start_pose=None, goal_pose=None):
    """Executes 2.5D LazyPRM* on the given map name."""
    loader = MapLoader()
    ...
    print(f"Executing 2.5D LazyPRM* (ArtPlanner) search on '{map_name}' from {start_pose[:2]} to {goal_pose[:2]}...")

    planner = LazyPRMStar25D(hmap, num_samples=1800)
    ...
    metrics["algorithm"] = "2.5D LazyPRM*"

    print("\n==========================================================================")
    print(f" KẾT QUẢ THỰC NGHIỆM 2.5D LAZYPRM* (ArtPlanner) (Map: {map_name})")
    print("==========================================================================")
    ...
    json_path = os.path.join(log_dir, f"lazy_prm_star_{map_name}.json")
    with open(json_path, "w") as f:
        json.dump(metrics, f, indent=2)

    img_path = os.path.join(log_dir, f"lazy_prm_star_{map_name}.png")
    ...
    if path:
        pts = np.array(path)
        ax1.plot(pts[:, 0], pts[:, 1], pts[:, 2] + 0.1, color='darkorange', linestyle='-.', linewidth=3, label='2.5D LazyPRM* Path')
    ...
    ax1.set_title(f'2.5D LazyPRM* (ArtPlanner) 3D Trajectory ({map_name})', fontsize=12, fontweight='bold')
    ...
    if path:
        pts = np.array(path)
        ax2.plot(pts[:, 0], pts[:, 1], color='darkorange', linestyle='-.', linewidth=2.5, label='2.5D LazyPRM*')
...
def main():
    print("==========================================================================")
    print("      2.5D LAZYPRM* (ArtPlanner - ETH Zurich) - STANDALONE                ")
    print("==========================================================================")
    loader = MapLoader()
    map_name, filepath = loader.select_map_interactive()
    run_lazy_prm_star_standalone(map_name)
